...
def mail(from_address, to_address, subject, msg, attachments=None, server=''):
    ...
    s = smtplib.SMTP()
    s.connect(server)
    s.sendmail(from_address, recipients,
               _generate_mail_msg(from_address, to_address, subject, msg, attachments))
    s.close()
...
def _construct_message_root(has_attachments, contains_html):
    # type: (bool, bool) -> message.Message
    if has_attachments:
...
def _separate_plain_and_html_parts(msg):
    # type: (Union[str, List, Tuple]) -> (str, str)
    msg_plain = msg
    msg_html = None
...
def _read_attachment(ctype, path):
    # type: (str, str) -> MIMEBase
...
def _process_one_attachment(path):
    ...
    msg_att.add_header('Content-Disposition', 'attachment',
