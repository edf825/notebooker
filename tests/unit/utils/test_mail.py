...
from man.notebooker.utils.mail import (_generate_mail_msg, mail, _SMTP_SERVER_ENV_KEY, _construct_message_root,
                                       _separate_plain_and_html_parts, _read_attachment, _process_attachments,
                                       _process_one_attachment, _construct_alternative_message_part)
try:
...
_MAIL_MODULE = 'man.notebooker.utils.mail.'


def test_plain_msg():
    """Plain mail msg generated properly"""
    expected = """Subject: this is the test subject\nFrom: sender@test.com\nTo: x@mi.com\n\nthis is \n the body"""
    assert expected == _generate_mail_msg("sender@test.com", "x@mi.com",
...
def test_plain_msg_several_recipients():
    ...
    expected = """Subject: this is the test subject\nFrom: sender@test.com\nTo: x@mi.com,y@mi.com\n\nthis is \n the body"""
    assert expected == _generate_mail_msg("sender@test.com", "x@mi.com,y@mi.com",
                            "this is the test subject", "this is \n the body")
...
--===============(\d)+==--"""
    ...
    msg = _generate_mail_msg("sender@test.com", "x@mi.com, y@mi.com",
                            "this is the test html subject",
                            ["this is \n the plain body", "<html>html test</html>"], None)
    assert re.match(expected, msg)


@pytest.mark.parametrize(('server_arg', 'expect_env_smtp'), [
    ('arg_smtp', False),
    (None, True)
])
def test_mail(server_arg, expect_env_smtp):
    ...
    with patch("smtplib.SMTP", return_value=s) as SMTP, \
        patch.dict('os.environ', {_SMTP_SERVER_ENV_KEY: 'env_smtp'}):
            mail('from_add', 'jbloggs@ahl.com', 'subject', 'msg', server=server_arg)
    ...
    assert s.sendmail.call_args_list == [call('from_add', ['jbloggs@ahl.com'], expected_msg)]


@pytest.mark.parametrize(('attachments', 'html', 'ctor', 'args'), [
...
@pytest.mark.parametrize(('msg', 'parts'), [
    ('plain', ('plain', None)),
    (['plain'], ('plain', None)),
    (('plain',), ('plain', None)),
...
@pytest.mark.parametrize(('ctype',  'ctor', 'arg'), [
...
def test_read_attachment(ctype, ctor, arg):
    ...
    with patch(_MAIL_MODULE + ctor) as mock_ctor, patch(compat_builtin) as mo, patch(_MAIL_MODULE + 'encoders'):
...
def test_process_attachments():
    ...
    with patch(_MAIL_MODULE + _construct_alternative_message_part.__name__) as camp, \
            patch(_MAIL_MODULE + _process_one_attachment.__name__) as poa,\
            patch('os.path.isfile') as is_file:
...
@pytest.mark.parametrize(('path', 'guess_type', 'call_args'), [
...
def test_process_one_attachment(path, guess_type, call_args):
    with patch(_MAIL_MODULE + _read_attachment.__name__) as ra, \
            patch('mimetypes.guess_type') as gt:
