...
def test_get_results_from_name_and_params():
    ...
    job_id_func.assert_called_once_with(
        sentinel.report_name,
        sentinel.report_params
    )


@patch('man.notebooker.utils.results._get_results_from_name_and_params')
...
@patch('man.notebooker.utils.results._get_job_results')
def test_get_latest_successful_job_results(_get_job_results):
    serializer = MagicMock()
